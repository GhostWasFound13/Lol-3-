...
.addFields([
    {
        name: "Usage", value: `\`${prefix}purge <amount>\` - Delete a number of messages.\n\`${prefix}purge <amount> --${commands.join(`\n\`${prefix}purge <amount> --`)}`
    }
])
.setFooter({ text:`${prefix}purge`}) 
...
await message.channel.bulkDelete(amount).then(async (m) => { 
  ...
  .setDescription(`✅  Cleared **${m.size}**/**${amount}** messages!`);
   ...
   message.channel.send({ embeds: [embed] }).then(msg => msg.delete({timeout:50000})); 
      })
